...
> 本节用于把原始小项目之后新增的真实代码能力同步回文档，作为课程设计报告取材边界。以后报告撰写和截图说明以本节与 `docs/2-designs/` 的对齐内容为准。

### 6.1 当前已实现业务能力
- **RBAC 与登录注册**：`/users/login`、`/users/register` 为认证白名单；注册必须提供用户名、密码、真实姓名和有效单位，后端强制新用户为 `role=0` 设备操作员。
- **四级角色模型**：`0-设备操作员`、`1-维修工程师`、`2-资产管理员`、`3-系统管理员`。前端路由通过 `meta.roles` 做页面拦截，后端通过 `LoginCheckInterceptor + BaseContext + @RequiresRoles` 做强制校验。
- **设备台账与生命周期**：设备支持分页检索、导出、折旧计算、详情聚合；详情聚合设备基础信息、折旧、领用、维修、调拨、报废和审计流水。
- **领用审批工作流**：无保管人设备通过 `t_equipment_claim` 记录申请、审批、撤回、退还和直接分配；当前状态口径以 `status='在用' && custodian IS NULL` 表示可领用，不单独落库“闲置”设备状态。
- **检修闭环**：报修、资产管理员指派、维修工程师完工、资产管理员复核恢复可用或转报废；工单状态为 `0-待指派, 1-维修中, 2-待复核, 3-已复核可用, 4-已复核转报废`。
- **审计、治理与消息中心**：关键业务动作写入 `operation_log`；治理模块提供质量问题、风险设备、异常维保成本等规则分析；消息中心基于规则生成高风险设备、待审批积压、维修超时提醒。
- **AI 辅助报告**：AI 模块基于看板、治理和设备生命周期 DTO/VO 生成周报/月报草案和单设备生命周期摘要；AI 只输出草案，不执行任何业务写操作。

### 6.2 当前实现边界
- 系统不引入独立“闲置”数据库状态；“可领用”由“在用且无保管人”表达。
- 消息中心采用页面拉取和手动已读，不使用 WebSocket、邮件、短信或企业微信。
- AI 输出不持久化，不直接访问数据库，不执行审批、报废、调拨、删除或数据库恢复。
- 数据库恢复、用户管理、审计日志等系统级能力限定系统管理员使用；资产管理员负责业务流转和本单位资产运营。
