...
- [ ] T1 — 更新 API 契约设计：更新 `docs/1-standards/design/api_contract.md` 补录 403 越权响应及相关参数规范 · covers: {AC-006}
- [ ] T2 — 升级 DB Schema 并编写物理表结构变更脚本：
...
- [ ] T3 — 新增自定义注解与异常类：创建 `RequiresRoles` 注解以及 `ForbiddenException` 异常类 · covers: {AC-001, AC-002}
- [ ] T4 — 实现 BaseContext 线程上下文及 AOP 拦截：
...
- [ ] T5 — 配置全局异常处理：在 `GlobalExceptionHandler` 中拦截 `ForbiddenException`，设置 HTTP 状态码为 403，并返回统一 Result 包体 · covers: {AC-001, AC-002}
- [ ] T6 — 控制器方法鉴权接入：在设备、维保、调拨、报废、主数据及备份还原 Controller 上加入 `@RequiresRoles` 注解进行写限制 · covers: {AC-001, AC-002}
- [ ] T7 — 设备与检修列表后端数据隔离实现：
...
- [ ] T10 — 编写集成与流转测试用例：编写 Service 层与 Controller 层的测试类，模拟不同角色鉴权，验证 P0/P1/P2 的全部越权和隔离场景 · covers: {AC-001, AC-002, AC-003, AC-004, AC-005, AC-006, AC-007, AC-008}
- [ ] T11 — 运行后端测试：执行 `cd equipment_system_management && mvn test` 保证所有测试通过 · covers: {AC-001, AC-002, AC-003, AC-004, AC-005, AC-006, AC-007, AC-008}
- [ ] T12 — 验证 ACs 状态：手动验证各 AC 并在 `spec.md` 中更新 passes 状态为 true · covers: {AC-001, AC-002, AC-003, AC-004, AC-005, AC-006, AC-007, AC-008}
- [ ] T13 — 更新进度规划：修改 `docs/3-tasks/CURRENT_PLAN.md` 标记本任务完成 · covers: {AC-006}
