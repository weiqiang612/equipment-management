...
- [x] T1 — 检查并确认前端调用 API 接口契约：检查 `api/equipment.js` 等接口定义与后端一致性 · covers: {AC-001, AC-002, AC-003, AC-004, AC-005, AC-006}
- [x] T2 — 修正路由拦截限制：修改 `router/index.js`，收紧调拨、报废、分类及单位页面的 roles 为 `[2, 3]` · covers: {AC-001}
- [x] T3 — 升级侧边栏动态菜单控制：修改 `App.vue` 侧边栏结构，限制维修工（1）和操作员（0）的菜单显示范围 · covers: {AC-002}
- [x] T4 — 改造设备台账页面操作栏显隐：
...
- [x] T7 — 多账户局部部署与手动联合功能校验：登录各个角色账号（0, 1, 2, 3）在浏览器中多视角验证路由拦截、侧边栏菜单隐藏、表格操作列显隐、指派下拉框及他人检修记录修改限制 · covers: {AC-001, AC-002, AC-003, AC-004, AC-005, AC-006}
- [x] T8 — 验证 ACs 状态：手动验证各 AC 并在 `spec.md` 中更新 passes 状态为 true · covers: {AC-001, AC-002, AC-003, AC-004, AC-005, AC-006}
- [x] T9 — 更新任务规划进度：修改 `docs/3-tasks/CURRENT_PLAN.md` 标记本任务完成
