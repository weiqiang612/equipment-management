  ...
  methods: {
    ...
    renderMarkdown(text) {
      return buildAiReportHtml({
        content: text,
        title: this.currentReport && this.currentReport.data ? this.currentReport.data.title : '',
        generatedTime: this.currentReport && this.currentReport.data ? this.formatTime(this.currentReport.data.generatedTime) : ''
      })
    },
    ...
    formatTime(timeArray) {
      return formatReportTime(timeArray)
    },
    exportPdf() {
      ...
      printAiReportFromFrame(printFrame, {
        title: report.title || this.currentPeriodLabel,
        badge: this.currentPeriodLabel,
        content: this.getSanitizedReportContent(report),
        generatedTime: this.formatTime(report.generatedTime),
        metaItems: [
          { label: '生成时间', value: this.formatTime(report.generatedTime) },
          { label: '输出性质', value: 'AI 建议草案' }
        ]
      })
    }
  }
